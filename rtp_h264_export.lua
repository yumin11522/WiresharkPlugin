...
do
    ...
    local function export_h264_to_file()
        ...
        local function process_single_nalu(stream_info, h264)
            write_to_file(stream_info, h264:tvb():raw(), true, true)
        end
        ...
        local function process_fu_a(stream_info, h264)
            ...
            if bit.band(fu_hdr, 0x80) ~= 0 then
                ...
                write_to_file(stream_info, string.char(nalu_hdr) .. h264tvb:raw(2), true, end_of_nalu)
            else
                -- start bit not set, just write part of nalu body
                write_to_file(stream_info, h264tvb:raw(2), false, end_of_nalu)
            end
        end
