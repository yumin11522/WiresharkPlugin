...
do
    ...
    if bit == nil then
        error("rtp_ps_no_assemble: bit operations library not available")
    end

    -- Cannot register two protocols with the same name/description (Wireshark 4.x).
    -- Prefer rtp_ps_assemble.lua when both are present in the plugins directory.
    local function dissector_exists(name)
        for _, n in ipairs(Dissector.list()) do
            if n == name then
                return true
            end
        end
